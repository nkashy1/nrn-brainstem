...
/**
 * Stimuli is intended for deployment by data trial principal investigator (PI). There should be
 * one Stimuli contract per data trial.
 *
 * The contract offers the following semantics:
 * 1. A user, when they consent to enrolling in a data trial, calls the trial's Stimuli contract
 ...
 *    the doc.ai application. This fires a Stimulus event.
 ...
 *    This enrollment bonus is specified as a variable on the Stimuli contract for that trial.
 ...
 *    method of this contract. Submission fires a Stimulus event.
 ...
 *    are specified in a rewards array publicly visible on the Stimuli contract.
 *
 * NOTE: The PI should approve the deployed Stimuli contract to pay out the appropriate amount
 * of NRN tokens.
 */
contract Stimuli {
    // State variables
    ...
    event Stimulus(address indexed _candidate, uint8 indexed _stimulusType, uint _stimulusId);
